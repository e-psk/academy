...
It is quite common for blacklist-based extensions check to forget to include `.htaccess` files. Unfortunately, uploading a .htaccess file can lead to Remote Code Execution (RCE) because this file is used to configure how the web server (usually Apache) processes requests.
...
To bypass the above check we need to prepare and upload a valid .htaccess file containing a malicious SetHandler attribute that would result in files with .jpg extension being executed as PHP code. Below is the example of such a .htaccess file:
